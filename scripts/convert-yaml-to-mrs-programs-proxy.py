#!/usr/bin/env python3
"""
Конвертирует rules/programs/programs_proxy.yaml в текстовый формат
для mihomo convert-ruleset (classical).

Читает: rules/programs/programs_proxy.yaml (с payload:)
Пишет: /tmp/combined-programs-proxy.txt в формате PROCESS-NAME,... или PROCESS-NAME-REGEX,...
...
RULES_FILE = Path("rules/programs/programs_proxy.yaml")
OUTPUT_FILE = Path("/tmp/combined-programs-proxy.txt")
